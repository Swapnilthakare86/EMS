...
function AttendanceAll() {
  ...
  function isLate(checkIn) {
    ...
    return new Date(`1970-01-01T${checkIn}Z`) > new Date("1970-01-01T04:30:00Z");
  }
  ...
  // ── Single-date rows (Employee role) ──────────────────────────────────────
  ...
  const singleDateRows = useMemo(() => {
    ...
    return employees.map(emp => {
      ...
      if (isFuture)
        return { ...emp, check_in: "-", check_out: "-", status: onLeave ? "L" : "-", total_hours: "-", late: false, halfDay: false };
  ...
  // ── Date-range rows (Admin / HR / Manager) ────────────────────────────────
  const rangeRows = useMemo(() => {
    ...
    return result;
  }, [isAdminHRManager, attendance, scopedEmployees, leaves, approvedStatusId, dateFrom, dateTo, todayStr]);
  ...
  const currentRows       = filteredRows.slice(indexOfLastRecord - recordsPerPage, indexOfLastRecord);
        ...
        <table className="table table-bordered table-hover">
          ...
          <tbody>
            {currentRows.length > 0 ? currentRows.map((row, idx) => (
              ...
              <tr>
                <td colSpan={isAdminHRManager ? (statusFilter === "L" ? 3 : 6) : (statusFilter === "L" ? 2 : 5)}
                  className="text-center text-muted">No records found</td>
              </tr>
            )}
          </tbody>
        </table>
